...
while running:
    ...
    for i in range(NUM_PARTICLES):
        ...
        # Vector from bowl centre to ball
        d = positions[i] - BOWL_CENTER
        distance = np.linalg.norm(d)

        if distance + PARTICLE_RADIUS > BOWL_RADIUS:
            
            n = d / distance

            velocities[i] -= (1 + WALL_RESTITUTION) * np.dot(velocities[i], n) * n

            offset = distance + PARTICLE_RADIUS - BOWL_RADIUS
    ...
    for i in range(NUM_PARTICLES):
            ...
            for j in range(i + 1, NUM_PARTICLES):
                ...
                if distance <= 2 * PARTICLE_RADIUS:
                    ...
                    a = np.dot(velocities[j], n)
                    b = np.dot(velocities[i], n)
    
                    if a - b < 0:
    
                        velocities[j] -= (
                            0.5 * (1 + RESTITUTION) * (a - b) * n
                        )
